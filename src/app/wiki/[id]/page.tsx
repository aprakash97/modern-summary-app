...
export default async function ViewArticlePage({
  ...
  const loggedInUser = await stackServerApp.getUser({ or: "redirect" });

  const isEditable = await authorizeToEditArticle(loggedInUser.id, +id);
  if (!article) {
    return <NotFound />;
  }
  ...
  return <WikiArticleViewer article={article} canEdit={isEditable} />;
}
